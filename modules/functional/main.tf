...
resource "azurerm_function_app" "function_app" {
  ...
  dynamic auth_settings {
    ...
    content {
      ...
      additional_login_params = try(auth_settings.value.additional_login_params, null)
      allowed_external_redirect_urls = try(auth_settings.value.allowed_external_redirect_urls, null)
      default_provider = try(auth_settings.value.default_provider, "AzureActiveDirectory")
      ...
      unauthenticated_client_action = try(auth_settings.value.unauthenticated_client_action, "AllowAnonymous")
      ...
      dynamic microsoft {
       ...
       dynamic twitter {
        ...
        content {
          ...
          consumer_secret = twitter.value.consumer_key
        }
      }
  ...
  dynamic source_control {
    ...
    content {
      repo_url = try(source_control.value.repo_url, null)
      branch   = try(source_control.value.branch, "master")
      manual_integration = try(source_control.value.manual_integration, false)
      rollback_enabled = try(source_control.value.rollback_enabled, false)
      use_mercurial = try(source_control.value.use_mercurial, false)
...
resource "azurerm_monitor_diagnostic_setting" "monitor_diagnostic_setting" {
  count                      = var.enable_functionapp_diagnostic ? 1 : 0
